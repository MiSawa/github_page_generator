...
\newcommand{\epsceil}[1]{\lceil {#1} \rceil_\epsilon}
$$</div>
...
この記事では, 辺費用が `$-C$` 以上の整数であること場合に適用できる, `$O(\sqrt{n} m \log(C))$` のアルゴリズムを, [この論文][paper]より紹介する.
このアルゴリズムの一種は, 最小費用流問題に対するコストスケーリング法の最適化, 具体的にはポテンシャルのみを変更することで scaling phase を飛ばすことが出来るか判定するために使われることがある.
個人的には, [Dilworthの定理][Dilworth]/[Mirskyの定理][Mirsky]の系を活用しているのが非常に面白いと思った.
...
頂点部分集合 `$S$` であって, 任意の相異なる2頂点 `$u, v \in S$` が `$u \succ v$` 又は `$v \succ u$` を満たすとき, `$S$` を **chain (鎖)** と呼ぶ.
また, 頂点部分集合 `$S$` であって, 任意の相異なる2頂点 `$u, v \in S$` が `$u \not\succ v$` かつ `$v \not\succ u$` を満たすとき, `$S$` を **antichain (反鎖)** と呼ぶ.

例えば, `$G$` のパスの頂点集合は chain であり, 入次数 `$0$` の頂点全体からなる集合は anti chain である.
さて, chain と antichain の(要素数の意味での)最大値に関して, 次の定理が成り立つ:
...
> `$G$` に新たに頂点 `$s$` を加え, `$s$` から(`$s$` 以外の)入次数 `$0$` の頂点へ辺を張る.
...
`$n$` 頂点 `$m$` 辺の有向グラフ `$G = (V, E)$` と, 辺費用 `$c(e) \in \Z$` が与えられる.
...
$$`
である.
従って, 閉路の被約費用はポテンシャルのとり方に依らず, またパスの被約費用はそのパスの始点と終点でのポテンシャルの値のみに依存する.
よって, `$s$`-`$t$` 最短経路は, その辺費用を `$c$` としても `$c^\pi$` としても変わらない.

ポテンシャル `$\pi$` が feasible であるとき, その定義から `$c^\pi(e) \ge 0$` であるから, 非負費用 `$c^\pi$` による最短経路問題を解けば, もとの費用での最短経路問題と同じ解を得ることが出来る.
非負費用であるとき, 最短経路問題は Dijkstra 法で `$O(m + n \log n)$` で解けることが知られているから, feasible なポテンシャルが求められればよいということになる.

上の議論では, feasible なポテンシャルの存在については言及していなかった.
例えば, 負閉路, すなわち `$c(C) < 0$` となる閉路 `$C$` があるとき, 任意のポテンシャル `$\pi$` は, `$c(C) = c^\pi(C) = \sum_{e \in C} c^\pi(e) < 0$` であるから `$c^\pi(e) < 0$` となる辺が存在し, feasible でない.
...
ポテンシャル `$\pi$` が, `$\epsilon > 0$` でこれを緩和した `$\forall e \in E,\ c^\pi(e) > -\epsilon$` を満たすとき, `$\pi$` は **`$\epsilon$`-feasible (`$\epsilon$`-実行可能)** であるという. [^eps feasibility def]
また, `$c^\pi(e) \le 0$` である辺を **admissible (使用可能, 又は許容)** であると言い, `$c^\pi(e) \le -\epsilon$` である辺を **`$\epsilon$`-improvable な頂点** であると言う. [^admissible def] また, `$\epsilon$`-improvable な辺 `$(u, v)$` が存在するような頂点 `$v$` を, やはり **`$\epsilon$`-improvable な頂点** であると言う.

`$\epsilon = 1$` のとき, `$\epsilon$`-feasible な*整数*ポテンシャルを `$\pi$` とすると,
`$c^\pi((u, v)) = c((u, v)) + \pi(u) - \pi(v) > -1$` であり, 左辺は整数であるから `$c^\pi(e) \ge 0$`, つまり `$\pi$` は feasible である.

入力の整数性と前節の feasible なポテンシャルの構築から, 整数ポテンシャルのみを考えてもよいことがわかる.
以下, 補足の節まで, ポテンシャルは整数ポテンシャルのみを考える.
...
コストスケーリング法は, この `$\epsilon$` とゼロポテンシャルを初期値とし, 次を繰り返すアルゴリズムである.
...
ポテンシャルを一つ固定し, Admissible な辺 `$E_p := \set{e \in E \setmid c^\pi(e) \le 0}$` のみを取り出したグラフ `$G^\pi = (V, E_p)$` を **admissible graph** と呼ぶ.
この `$G^\pi$` の任意の閉路 `$C$` のコストは `$c(C) = c^\pi(C) = \sum_{e \in C} c^\pi(e) \le 0$` であり, `$C$` が負の被約費用を持つ辺を含むならば `$C$` は負閉路である.
従って, `$G^\pi$` を強連結成分分解し, 各強連結成分に負の被約費用を持つ辺が存在するかを判定し, 存在するならばアルゴリズムを終了してよい.

そうでないとき, 各強連結成分内の辺の被約費用は `$0$` である.
この強連結成分内の辺全体の集合を `$S$` とする.
`$G$` をもとにし, 強連結成分たちを縮約したグラフ `$H$` を考えると, `$H$` 上のポテンシャル `$\rho$` と `$G$` 上の `$\forall e \in S,\ c^\rho(e)=0$` であるようなポテンシャル `$\rho$` は, その整数性, feasibility や `$\epsilon$`-feasibility を保ったまま 1:1 対応し, `$H$` の `$\pi$` に対応するポテンシャルでの admissible graph は DAG になる.

従って, 必要ならば強連結成分分解をすることで, admissible graph は DAG であるとしてよい. [^decycle not needed]

[^decycle not needed]: `$\epsilon$`-feasibility の定義を変えると, decycle が必要なくなる一方で, 別の面倒が発生する. 詳しくは補足の節で.


## Cut-Relabel
さて, `$(2\epsilon)$`-feasible なポテンシャル `$\pi$` と, 付随する DAG である admissible graph `$G^\pi$` を考える.
頂点集合 `$S$` に対し, `$G^\pi$` 上で `$S$` から辿り着ける頂点全体を `$S$` の閉包と呼び `$\overline{S}$` で表す. `$S = \overline{S}$` であるとき `$S$` を閉集合であるという. 任意の頂点集合 `$S$` に対し, `$\overline{S}$` は閉集合である.
...
また, 更新前の `$\pi$` が `$\epsilon$` の整数倍であるならば, 更新後も `$\epsilon$` の整数倍である.
...
したがって, このような頂点集合 `$X$` に含まれる improvable な頂点は, `$\operatorname{CUT\_RELABEL}(\overline{X})$` によって improvable でなくなる.

これにより, Refine の実現として, 次のアルゴリズムが考えられる.

1. 必要ならば Decycle する.
2. improvable な頂点が存在しなければ, `$\pi$` は `$\epsilon$`-feasible であり, 終了する. そうでないとき, improvable な頂点をひとつ取り `$x$` とする.
...
`$P$` を `$G^\pi$` のパスとする.
パスにこの順で含まれる頂点 `$u, v$` に対し, `$P_v$` をこのパスの始点から `$v$` までの部分パスとし, `$P_{u,v}$` を `$u$` から `$v$` までの部分パスとする.
...
一方で, `$\pi$` が `$1$`-feasible な整数ポテンシャルであっても, feasible とは限らなくなる.
